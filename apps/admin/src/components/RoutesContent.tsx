    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1rem" }}>
        ...
        {!loading && routes.map((r) => (
          <div key={r.id} style={{ background: "#fff", borderRadius: "14px", padding: "1.5rem", border: "1px solid #e8e4dc" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1rem" }}>
              <div>
                <div style={{ fontWeight: 500, fontSize: "0.95rem", marginBottom: "2px" }}>{r.origin}</div>
                <div style={{ color: "var(--brand-gold)", marginBottom: "2px" }}>↓</div>
            ...
            <div style={{ display: "flex", gap: "1.5rem", paddingTop: "1rem", borderTop: "1px solid #f0ece4", fontSize: "0.8rem", color: "var(--brand-gray)" }}>
              ...
              <button onClick={() => openEdit(r)} style={iconBtn}>✏️</button>
              <button onClick={() => handleDelete(r)} style={iconBtn}>🗑️</button>
            </div>
          </div>
        ))}
      </div>
...
const btnSecondary: React.CSSProperties = {
  padding: "8px 16px", borderRadius: "8px", border: "1px solid #e0dbd0",
  background: "#fff", fontSize: "0.875rem", cursor: "pointer",
};
