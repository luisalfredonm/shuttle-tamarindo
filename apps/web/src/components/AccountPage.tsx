  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2.5rem 2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', marginBottom: '0.3rem' }}>My Bookings</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        {[
          ...
          <div key={s.label} style={{ background: '#fff', borderRadius: '14px', padding: '1.25rem', border: '1px solid #e8e4dc', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--brand-gray)', fontFamily: 'DM Sans, sans-serif' }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', borderBottom: '1px solid #e8e4dc', marginBottom: '1.5rem' }}>
      ...
      {loading && <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--brand-gray)', fontFamily: 'DM Sans, sans-serif' }}>Loading your bookings...</div>}

      {!loading && displayed.length === 0 && (
        <div style={{ background: '#fff', borderRadius: '16px', padding: '3rem', textAlign: 'center', border: '1px solid #e8e4dc' }}>
          <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>{tab === 'upcoming' ? '🗓' : '📋'}</div>
          <h3 style={{ fontFamily: 'DM Sans, sans-serif', marginBottom: '0.5rem' }}>
            {tab === 'upcoming' ? 'No upcoming trips' : 'No past trips'}
          </h3>
          <p style={{ color: 'var(--brand-gray)', fontFamily: 'DM Sans, sans-serif', fontSize: '0.9rem', marginBottom: '1.5rem' }}>
            {tab === 'upcoming' ? 'Book your next transfer to get started.' : 'Your completed trips will appear here.'}
          </p>
          {tab === 'upcoming' && (
            <Link href="/#book" style={{ background: 'var(--brand-green)', color: '#fff', padding: '11px 24px', borderRadius: '8px', fontFamily: 'DM Sans, sans-serif', fontWeight: 500, fontSize: '0.9rem', textDecoration: 'none' }}>
              Book a Transfer
            </Link>
          )}
        </div>
      ...
      {!loading && displayed.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {displayed.map(booking => <BookingCard key={booking.id} booking={booking} />)}
        </div>
      )}
    </div>
  );
...
function BookingCard({ booking }: { booking: Booking }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e8e4dc', overflow: 'hidden', opacity: isPast && booking.status === 'CANCELLED' ? 0.6 : 1 }}>
      <div style={{ background: isPast ? '#f5f4f0' : 'var(--brand-dark)', padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
        ...
        <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.8rem', color: isPast ? 'var(--brand-gray)' : 'rgba(255,255,255,0.5)' }}>
      ...
      <div style={{ padding: '1.25rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
          {[
            ...
            <div key={item.label}>
              ...
              <div style={{ fontFamily: 'DM Sans, sans-serif', fontWeight: 500, fontSize: '0.9rem', color: 'var(--brand-dark)' }}>{item.value}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          {booking.status === 'PENDING' && (
            <button onClick={() => router.push('/payment?bookingId=' + booking.id)} style={{ background: 'var(--brand-gold)', color: 'var(--brand-dark)', border: 'none', borderRadius: '8px', padding: '9px 18px', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 500, fontSize: '0.875rem' }}>
              Pay Now
            </button>
          )}
          ...
          {booking.status === 'CONFIRMED' && (
            <Link href={'/booking-success?bookingId=' + booking.id} style={{ border: '1px solid #e0ddd6', borderRadius: '8px', padding: '8px 16px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.875rem', color: 'var(--brand-gray)', textDecoration: 'none' }}>
              View
            </Link>
          )}
        </div>
